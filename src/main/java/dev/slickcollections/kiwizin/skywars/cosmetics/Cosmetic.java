...
public abstract class Cosmetic {
  ...
  public void give(Profile profile) {
    profile.getAbstractContainer("kCoreSkyWars", "cosmetics", CosmeticsContainer.class).addCosmetic(this);
  }
  
  public boolean has(Profile profile) {
    return profile.getAbstractContainer("kCoreSkyWars", "cosmetics", CosmeticsContainer.class).hasCosmetic(this);
  }
  
  public boolean isSelected(Profile profile) {
    return profile.getAbstractContainer("kCoreSkyWars", "selected", SelectedContainer.class).isSelected(this);
  }
  ...
  public boolean isSelectedPerk(Profile profile) {
    ...
    for (int i = 1; i < checkIndex + 1; i++) {
      ...
      Cosmetic cosmetic =
          profile.getAbstractContainer("kCoreSkyWars", "selected", SelectedContainer.class).getSelected(CosmeticType.PERK, Perk.class, i);
      if (cosmetic == null) {
        continue;
      }
      if (cosmetic == this) {
  ...
  public int getAvailableSlot(Profile profile) {
    ...
    for (int i = 1; i < checkIndex + 1; i++) {
      Cosmetic cosmetic =
          profile.getAbstractContainer("kCoreSkyWars", "selected", SelectedContainer.class).getSelected(type, Perk.class, i);
      if (cosmetic != null) {
        i++;
      } else {
        isSelected = i;
